...
const Components = {
    ...
    header: (user, currentPage = 'dashboard') => {
        ...
        const isAdmin = user.is_admin === 1 || user.is_admin === true || user.username === 'alice';
        
        // Build dropdown items
        let dropdownItems = `
            <div class="dropdown-item ${currentPage === 'dashboard' ? 'active' : ''}" onclick="Components.navigateTo('dashboard', ${JSON.stringify(user).replace(/"/g, '&quot;')})">
                <span class="item-icon">📊</span>
                <span>Dashboard</span>
            </div>
            <div class="dropdown-item ${currentPage === 'transfer' ? 'active' : ''}" onclick="Components.navigateTo('transfer', ${JSON.stringify(user).replace(/"/g, '&quot;')})">
                <span class="item-icon">↔️</span>
                <span>Transfer</span>
            </div>
            <div class="dropdown-item ${currentPage === 'withdraw' ? 'active' : ''}" onclick="Components.navigateTo('withdraw', ${JSON.stringify(user).replace(/"/g, '&quot;')})">
                <span class="item-icon">📤</span>
                <span>Withdraw</span>
            </div>
            <div class="dropdown-item ${currentPage === 'history' ? 'active' : ''}" onclick="Components.navigateTo('history', ${JSON.stringify(user).replace(/"/g, '&quot;')})">
                <span class="item-icon">📜</span>
                <span>History</span>
            </div>
        `;
        
        // Add admin panel if user is admin - NOW POINTS TO ENHANCED ADMIN
        if (isAdmin) {
            dropdownItems += `
                <div class="dropdown-item" onclick="window.location.href='/admin-ultimate.html'">
                    <span class="item-icon">👑</span>
    ...
    balanceCards: (user) => {
        ...
        return `
            <div class="balance-grid">
                <div class="balance-card sol" onclick="Components.navigateTo('withdraw', ${JSON.stringify(user).replace(/"/g, '&quot;')})">
                ...
                <div class="balance-card usdc" onclick="Components.navigateTo('withdraw', ${JSON.stringify(user).replace(/"/g, '&quot;')})">
                    <div class="balance-icon">💵</div>
