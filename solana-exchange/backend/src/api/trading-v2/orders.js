...
// ============================================
// UPDATED CLOSE POSITION - KEEPS MARGIN, ONLY RETURNS PROFIT
// ============================================
router.post('/close', auth, async (req, res) => {
    ...
    try {
        ...
        const position = await new Promise((resolve, reject) => {
            db.get(
                'SELECT * FROM trading_v2_positions WHERE id = ? AND user_id = ? AND status = "OPEN"',
                [positionId, userId],
                (err, row) => {
                    if (err) reject(err);
                    else resolve(row);
        ...
        // IMPORTANT: Only return profit if positive, NEVER return margin!
        const returnAmount = pnl > 0 ? pnl : 0;
        
        // Return ONLY profit to trading balance (if any)
        if (returnAmount > 0) {
            await new Promise((resolve, reject) => {
                db.run(
                    'UPDATE trading_v2_balances SET usdc_balance = usdc_balance + ? WHERE user_id = ?',
        ...
        // Update position - margin is NEVER returned!
        await new Promise((resolve, reject) => {
            db.run(
...
router.get('/positions', auth, async (req, res) => {
    ...
    try {
        const positions = await new Promise((resolve) => {
            db.all(
                'SELECT * FROM trading_v2_positions WHERE user_id = ? AND status = "OPEN" ORDER BY created_at DESC',
                [req.userId],
